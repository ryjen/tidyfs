...
fn committed_live_evaluation_corpus_is_versioned_and_valid() {
    ...
    let fixture_ids: BTreeSet<_> = suite.fixtures.iter().map(|fixture| fixture.id.as_str()).collect();
    let baseline_ids: BTreeSet<_> = baseline
        .fixture_minimum_scores
        .keys()
        .map(String::as_str)
        .collect();
    assert_eq!(baseline_ids, fixture_ids, "every fixture needs a pinned score threshold");
}
